...
1. Run `./1_cache_mangas` to get local list of all mangas on the site (16 hours)
3. Run `./2_calc_similar` to generate similarity index between manga (1.0 hours)
4. Run `./3_calc_mappings` to export external ID to mangadex UUID mapping files (2 minutes)
4. Run `./4_calc_mu_ids` convert MangaUpdate ids into uniform "new" ids for their new api (24 hours)
