...
func (a *MangaApiService) GetSearchManga(ctx context.Context, localVarOptionals *MangaApiGetSearchMangaOpts) (MangaList, *http.Response, error) {
	...
	if localVarOptionals != nil && localVarOptionals.Order.IsSet() {
		//localVarQueryParams.Add("order", parameterToString(localVarOptionals.Order.Value(), ""))
		order := localVarOptionals.Order.Value().(map[string]string)
		for k, v := range order {
			localVarQueryParams.Add("order["+k+"]", parameterToString(v, ""))
		}
	}
	if localVarOptionals != nil && localVarOptionals.Includes.IsSet() {
		// todo: future self, can we make arrays be smarter?
 ...
 * @param optional nil or *MangaApiPutMangaIdOpts - Optional Parameters:
     * @param "Body" (optional.Interface of MangaIdBody) -  The size of the body is limited to 16KB.
@return MangaResponse
*/
